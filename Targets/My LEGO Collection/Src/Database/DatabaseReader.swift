...
struct DatabaseReader {
    ...
    init(forDatabaseAt url: URL) {
        ...
        self.connection = SQLite_Connection(toDatabaseAt: url)
    }
    ...
    func readAllColors() -> [LEGO_Color] {
        ...
        let colors = connection.readResults(of: "SELECT * FROM Colors;", with: { color(fromResultsOf: $0) })
    ...
    func readAllParts() -> [LEGO_Part] {
        ...
        let parts = connection.readResults(of: "SELECT * FROM Parts;", with: { part(fromResultsOf: $0) })
...
//private extension DatabaseReader {
//
//
...
private extension DatabaseReader {
    ...
    ///
    private func readRGB(from statement: SQLite_Statement, at index: Int) -> RGB {
        
        let raw = statement.readString(at: index)
    ...
    ///
    private func readOptionalURL(from statement: SQLite_Statement, at index: Int) -> URL? {
        
        if let raw = statement.readOptionalString(at: index) {
    ...
    ///
    private func color(fromResultsOf statement: SQLite_Statement) -> LEGO_Color {
        
        let name = statement.readString(at: 0)
        let rgb = readRGB(from: statement, at: 1)
        let transparent = statement.readBool(at: 2)
    ...
    ///
    private func part(fromResultsOf statement: SQLite_Statement) -> LEGO_Part {
